...
- **Notes**: Create text notes with title and content, optionally linked to tasks; available in both individual and team modes with full markdown support and live preview; attach up to 10 images per note (JPEG/PNG/GIF/WebP, 5 MB each) with a gallery in the note detail view
...
- **Notes**: Notes can be created independently or linked to specific tasks; team members can view all team notes, while only the author or team admin can edit or delete them. Notes support full markdown functionality with live preview. Note images are served through a private view (no direct MEDIA_URL access) and files are cleaned up when a note or image is deleted
